...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, adversarial, adv_eps, adv_eps_s, num_adversaries, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if local_q_func:
            # local_q_func is defaulted as false
...
class MADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, model, obs_shape_n, act_space_n, agent_index, args, local_q_func, policy_name, adversarial, noise_type, noise_std):
        self.name = name
        # each agent has its own name
        self.scope = self.name + "_" + policy_name
        # each agent has its own scope name for tf
        self.n = len(obs_shape_n)
        # n is num of all agents in a game
        ...
        for a in range(num_sample):
            ...
            for i in range(self.n):
                if i == self.agent_index:
                    ...
                    target_act_next_n.append(agents[i].p_debug["par_target_act"](obs_next_n[i]))
